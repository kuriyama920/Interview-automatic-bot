...
### shared.ts (source of truth — 全プロセス共通)
...
interface UserUsage {
  sttMinutes: number; aiTokens: number; storageBytes: number
}
interface User {
...
interface TranscriptResult {
  ...
  timestamp: number; source?: 'mic' | 'system'
}
interface AIResponse {
  answer: string; suggestions: string[]; confidence: number
}
interface GenerateOptions {
  includeDocumentContext?: boolean; maxTokens?: number
  turnId?: string; speculativeText?: string
}
...
interface DocumentInfo {
  ...
  uploadedAt: number; chunkCount: number
}
interface InterviewQuestion {
  id: string; question: string; answer: string
  sortOrder: number; isAutoGenerated: boolean
...
### auth.ts (認証固有のみ。共通型は shared.ts を参照)
...
interface DocumentMetadata {
  ...
  uploadedAt: number; chunkCount: number; totalTokens: number
}
```

### question.ts (shared.ts と同一定義を再エクスポート)
...
type Env = {
  ...
  CF_ACCOUNT_ID?: string; CF_AI_GATEWAY_ID?: string  // Cloudflare AI Gateway (optional)
}
type Variables = {
...
2. parseDocument() → text (pdf-parse or jszip)
