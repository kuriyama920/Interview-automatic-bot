...
describe('useQuestionCache', () => {
  ...
  it('should return findMatch, refreshCache, clearCache', () => {
...
describe('getCachedBigrams (bigram memoization)', () => {
  beforeEach(() => {
    clearBigramCache()
  })

  it('should return the same Set reference for the same text on multiple calls', () => {
    const first = getCachedBigrams('自己紹介をお願いします')
    const second = getCachedBigrams('自己紹介をお願いします')
    // Same reference means it was cached, not recomputed
    expect(first).toBe(second)
  })

  it('should return correct bigrams (same as computeBigrams)', () => {
    const cached = getCachedBigrams('abc')
    const direct = computeBigrams('abc')
    expect(cached).toEqual(direct)
  })

  it('should return different Set references for different texts', () => {
    const a = getCachedBigrams('テスト質問')
    const b = getCachedBigrams('別の質問')
    expect(a).not.toBe(b)
  })

  it('should evict oldest entry when cache exceeds 200 entries', () => {
    // Fill cache with 200 unique entries
    for (let i = 0; i < 200; i++) {
      getCachedBigrams(`text-${i}-pad`)
    }

    // Capture reference to the first (oldest) entry
    const refBefore = getCachedBigrams('text-0-pad')

    // Adding 201st entry should trigger eviction of the oldest (text-1-pad)
    // Since text-0-pad was just re-accessed, text-1-pad is now oldest in insertion order
    // But Map preserves insertion order, so text-0-pad is still first.
    // After eviction of text-0-pad, adding overflow entry takes its slot.
    getCachedBigrams('overflow-trigger-text')

    // text-0-pad was the oldest entry and should have been evicted.
    // Calling getCachedBigrams again should recompute and return a new reference.
    const refAfter = getCachedBigrams('text-0-pad')
    expect(refBefore).not.toBe(refAfter)
    // Values should still be semantically equal
    expect(refBefore).toEqual(refAfter)
  })

  it('should reuse cached bigrams for findMatch calls with same query', async () => {
    ;(window.electron.questions.list as ReturnType<typeof vi.fn>).mockResolvedValue({
      success: true,
      questions: [
        { question: '志望動機を教えてください', answer: '御社の成長性に惹かれました', isAutoGenerated: false },
      ],
    })

    const { result } = renderHook(() => useQuestionCache())
    await waitFor(() => expect(window.electron.questions.list).toHaveBeenCalled())

    // Clear bigram cache to start fresh
    clearBigramCache()

    const query = '志望動機を教えてください'

    // Call findMatch twice - second call should use cached bigrams
    result.current.findMatch(query)
    const cachedRef = getCachedBigrams(query)

    result.current.findMatch(query)
    const cachedRefAfter = getCachedBigrams(query)

    // Same reference means the bigram was computed once and reused
    expect(cachedRef).toBe(cachedRefAfter)
  })
})
